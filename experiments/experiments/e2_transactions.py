...
def experiment() -> None:
    ...
    parser.add_argument(
        "num_transaction_repetitions",
        type=int,
        choices=[1, 5, 15, 25],
        help="number of transaction repetitions. should be 1, 5, or 25, since each rep includes 5 transactions",
    )
    args = parser.parse_args()
    num_transaction_repetitions = args.num_transaction_repetitions
    with Runner(
        f"e2-transactions-{num_transaction_repetitions}",
        ...
        for account in admin_user.account_id_to_account.values():
            txn_hash = runner.deposit_into_account(account, Decimal("100"))
            runner.wait_for_tx(account.currency, txn_hash)
        ...
        withdrawal_amount = Decimal("0.02")
        ...
        for _ in range(num_transaction_repetitions):

            currencies_and_txn_hashes.clear()
            ...
            with ManagedThreadPool(MAX_WORKERS) as eth_pool, ManagedThreadPool(MAX_BTC_WORKERS) as btc_pool:
                for user in runner.users:
                    ...
                    for account in user.account_id_to_account.values():
                        ...
                        def do_deposit(account: Account = account) -> None:
                            def underlying() -> None:
            ...
            for currency, block_number in max_block_to_currency.items():
                runner.ensure_block_processed(
                    currency, minimum_block_number=block_number, timeout=timedelta(minutes=60)
                )

            withdrawal_addresses.clear()
            ...
            with ManagedThreadPool(MAX_BTC_WORKERS) as pool:
                for user in runner.users:
                    ...
                    def do_withdrawals_and_exchanges(user: User = user) -> None:
                        ...
                        runner.exchange(
                            user,
                            user.currency_and_account_type_to_accounts["ETH", AccountType.DEPOSIT_ACCOUNT][
                                0
                            ].account_id,
                            user.currency_and_account_type_to_accounts["GUSD", AccountType.DEPOSIT_ACCOUNT][
                                0
                            ].account_id,
                            Decimal("0.05"),
                        )
                        dest_address = runner.withdraw(
                            user,
                            user.currency_and_account_type_to_accounts[
                                withdrawal_currency, AccountType.DEPOSIT_ACCOUNT
                            ][0].account_id,
                            withdrawal_amount,
                        )
            ...
            runner.ensure_block_processed("ETH", timeout=timedelta(minutes=240), minimum_block_number=eth_block)
            runner.ensure_block_processed("GUSD", timeout=timedelta(minutes=3), minimum_block_number=eth_block)
            runner.ensure_block_processed("BTC", timeout=timedelta(minutes=240), minimum_block_number=btc_block)
